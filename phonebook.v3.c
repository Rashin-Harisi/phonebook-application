...
//In this version firstname and lastname length is not fixed and allocate memory based on the length of input.
...
void freeAllContacts(int count);
...
void addNewContact(int *count){
    if(*count >= capacity ){
        ...
        printf("First name : ");
        scanf("%199s",temp_buffer);
        users[*count].firstName = malloc(strlen(temp_buffer)+1);
        if(users[*count].firstName == NULL){
            printf("Allocation memory failed.\n");
            exit(1);
        }
        strcpy(users[*count].firstName, temp_buffer);
        printf("\n");
        printf("Last name : ");
        scanf("%199s",temp_buffer);
        users[*count].lastName = malloc(strlen(temp_buffer)+1);
        if(users[*count].lastName == NULL){
            printf("Allocation memory failed.\n");
            free(users[*count].firstName);
            exit(1);
        }
        strcpy(users[*count].lastName, temp_buffer);
        printf("\n");
        printf("Number : ");
...
void freeAllContacts(int count) {
    for (int i = 0; i < count; i++) {
        free(users[i].firstName);
        free(users[i].lastName);
    }
}